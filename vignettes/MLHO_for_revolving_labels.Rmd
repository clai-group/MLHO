...
For multiple exposures/treatments, we want to create the unique identifier for patients with different `start_date`. (Make sure the `start_date` in `labelDT` is the same as the start_date in `dbmart` when creating the unique identifier.)
```{r}
labelDT = labelDT %>%
  mutate(patient_num2 = patient_num,
         patient_num = paste0(patient_num, as.Date(start_date))) %>%
  distinct()
...
encounterLevel=F
valuesToMerge = F
timeBufffer=c(h=0,p=0,l=0,o=-14)
